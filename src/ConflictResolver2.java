...
public class ConflictResolver2 {
    ...
    private static Map<Integer, CompleteFlightInfo> unscheduled = new HashMap<Integer, CompleteFlightInfo>(); // store the unresolved flights schedule
    private static Map<Integer, CompleteFlightInfo> scheduled = new HashMap<Integer, CompleteFlightInfo>();  // store the resolved schedule
    private static int runwayId[] = new int[4];
    private static String arrCat[] = new String[4];	// store last arrival category on the runway i
    private static java.sql.Time arrTime[] = new java.sql.Time[4];	// store last arrival time on the runway i
    private static String depCat[] = new String[4];	// store last departure category on the runway i
    private static java.sql.Time depTime[] = new java.sql.Time[4];	// store last departure time on the runway i
    private static Integer schCounter = 0;	// counter for schedule hashmap
    private static long currentTime = 0;	// stores the current time, it increments by 60 after every scheduled flights
    private static java.sql.Time startTime;	// start time of the current schedule
    private static List<Long> allotedTimeInSec = new ArrayList<Long>();	// stores the time in sec after a flight has been scheduled
    private static List<java.sql.Time> allotedTime = new ArrayList<java.sql.Time>();
    private static int noOfFlights;
    ...
    public static Map<Integer, CompleteFlightInfo> resolver(CompleteFlightInfo[] cfi, int n){
        ...
        boolean isSchedule;	// check whether flight in focus is scheduled or not
        try{
            ...
            while(!unscheduled.isEmpty()){
                ...
                while(!isSchedule){
                    ...
                    if(counter <= scopeEnd || unscheduled.get(counter).arrivalDeparture.equals("D")){
                        boolean moveToSchedule = checkFlight(unscheduled.get(counter), createStatement);
                        if(moveToSchedule){
                            updateLastEntryTable(counter, createStatement);
            ...
            fillFLightLogTable(createStatement);
            schCounter = 0;
            return scheduled;
    ...
    public static void fillFLightLogTable(CreateStatement createStatement) {
    ...
    public static boolean checkFlight(CompleteFlightInfo cfi, CreateStatement createStatement){
    ...
    public static void updateLastEntryTable(int counter, CreateStatement createStatement){
