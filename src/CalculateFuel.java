...
public class CalculateFuel {
    ...
    public void runCode(int n){
        ...
        noOfFlights = n;
        findCoordinates(new CreateStatement());
        calculateDistInAir();
        FetchWind(new CreateStatement());
        calReqRunwayLen(new CreateStatement());
        calculateTaxiTimeTakeoff(new CreateStatement());
        calculateTaxiTimeLanding(new CreateStatement());
        checkValidity(new CreateStatement());
        //calculateTaxiTimeLanding();
        fuelCalculate(new CreateStatement());
        findTime(new CreateStatement());
        fuelOptimizer();
        display();
        deleteFromFlightInfoTable(new CreateStatement());
    }


    public void deleteFromFlightInfoTable(CreateStatement createStatement) {
    ...
    public static void checkValidity(CreateStatement createStatement){
        ...
        try{
            ...
            for(int j=0; j<4; j++){		// column wise traversal to find invalid runways for departures
                for(int i = 0; i < noOfFlights; i++){
                    if(!runwayvalidity[i][j] && arrivalDeparture[i].equals("D")){
            ...
            for(int i = 0; i < noOfFlights; i++){
                for(int j = 0; j < 4; j++){
                    if(runwayvalidity[i][j] && arrivalDeparture[i].equals("A")){
                        flag++;
    ...
    public static void FetchWind(CreateStatement createStatement){
    ...
    public static void calReqRunwayLen(CreateStatement createStatement){
        double length = 0;
        try{
            Statement stt = createStatement.getStatement();
            stt.execute("USE flextracks");
            ...
            for(int i = 0 ; i < noOfFlights; i++){
                ...
                while(res.next()){
                    ...
                    for(int j = 0; j < 4; j++){
                        if(runwayWind[j] <= 0){
                            runwayLenReq[i][j] = length * (1 + runwayWind[j] * 0.015);
                        }else{
                            runwayLenReq[i][j] = length * (1 + runwayWind[j] * 0.05);
                        }
                    }
                }
            }
    ...
    public static void calculateTaxiTimeTakeoff(CreateStatement createStatement){  // terminal to runway start
    ...
    public static void calculateTaxiTimeLanding(CreateStatement createStatement){
    ...
    public static void fuelCalculate(CreateStatement createStatement){
                ...
                for(int j = 0; j < noOfFlights; j++){
                    if(arrivalDeparture[j].equals("A")){
                        ...
                        for(int k = 0; k < 4; k++){ // k = runways
                            if(fuelConsumption[j][k] < minval && combinationRunwayMap[i][k] && fuelConsumption[j][k] != -1){
    ...
    public static void findCoordinates(CreateStatement createStatement){
    ...
    public static void findTime(CreateStatement createStatement){
        try{
            Statement stt = createStatement.getStatement();
            stt.execute("USE flextracks");
